...
// MARK: - Category
enum class FoodCategory(val raw: String, val emoji: String, val titleKey: String) {
    ALL("هەموو", "🛒", "cat.all"),
    FOOD("خواردن", "🍽", "cat.food"),
    DRINK("نەساتل", "🧃", "cat.drink"),
    FRIDGE("بەراد", "❄️", "cat.fridge");

    val title: String get() = L(titleKey)
...
data class FoodItem(
    ...
    val statusText: String
        get() {
            val d = daysLeft
            if (d < 0) return L("status.expired")
            if (d == 0) return L("status.today")
            if (d == 1) return L("status.oneDayLeft")
            if (d < 7) return L("status.daysLeft", d)
            val w = d / 7
            val r = d % 7
            return if (r == 0) L("status.weeksLeft", w) else L("status.weeksDaysLeft", w, r)
        }
...
object FoodStorage {
    ...
    /** هەموو خواردنەکان دەگۆڕێتەوە بە لیستێکی نوێ — بەکاردێت لە گەڕاندنەوەی باکئەپ */
    fun replaceAll(newItems: List<FoodItem>) {
